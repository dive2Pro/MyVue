 ...
 * @returns {any}
 */
function $expr(expr, target?):object {
    const p = expr.split(".");
...
function $watch(expr: string, callback: () => void, imme?: boolean): ObservableView {
    ...
    function activeNestObject(property) {
        ...
        (typeof parent == 'object') && Object.keys(parent)
            .filter(key => parent.hasOwnProperty(key)).forEach(key => {
    ...
    returned.data;
    if(imme){
        returned.simpleEventEmiiter.emit("")
    }
    return returned
}
    ...
    function makeObserveable<P>(target, child?: boolean): any {
        ...
        function extendObj(obj) {
            ...
            if (!child) {
                ...
                newObj.$exprset = $exprset.bind(newObj);
